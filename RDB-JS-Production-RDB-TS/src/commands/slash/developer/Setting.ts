...
export default new CommandExecutor()
	...
	.addStringOption(op =>
		op.setName('setting_name')
			.setDescription('The name of the setting')
			.setRequired(true))
	.addStringOption(op =>
		op.setName('setting_type')
	...
	.setExecutor(async (interaction) => {
		...
		let trueData: string | number | boolean = ''
		switch (settingType) {
			case 'string': {
			...
			case 'boolean': {
				const formattedValue = (settingValue as string).toLowerCase().trim()
