...
function parseErrorEntries(stdout: string): string[] {
...
function buildInfoEmbed(interaction: any): EmbedBuilder {
...
function buildErrorEmbed(entry: string, index: number, total: number): EmbedBuilder {
	return new EmbedBuilder()
...
function buildErrorNavRow(page: number, total: number): ActionRowBuilder<ButtonBuilder> {
	return new ActionRowBuilder<ButtonBuilder>()
		.addComponents(
			new ButtonBuilder()
...
function execLogs(command: string): Promise<string> {
...
export default new CommandExecutor()
	...
	.setExecutor(async (interaction) => {
		const isDeveloper = config.devs.includes(interaction.user.id)
		const infoEmbed = buildInfoEmbed(interaction)
		const sentMessage = await interaction.reply({ embeds: [infoEmbed], components: isDeveloper ? [buildOpenRow() as any] : [], fetchReply: true }) as Message
		...
		const replyMessage = sentMessage as any
		const collector = replyMessage.createMessageComponentCollector({ time: 120000 })

		collector.on("collect", async (buttonInteraction: ButtonInteraction) => {
			if (!config.devs.includes(buttonInteraction.user.id)) {
			...
			switch (buttonInteraction.customId) {
				case "info_errors_open": {
					if (!showingErrors) {
						...
						replyMessage.edit({ embeds: [errorEmbed], components: [navRow as any] })
					}
					break
				}
				case "info_errors_prev": {
					if (!showingErrors || currentPage === 0) return
					currentPage -= 1
					const pageEmbed = buildErrorEmbed(errorEntries[currentPage], currentPage, errorEntries.length)
					const navRow = buildErrorNavRow(currentPage, errorEntries.length)
					replyMessage.edit({ embeds: [pageEmbed], components: [navRow as any] })
					break
				}
				case "info_errors_next": {
					if (!showingErrors || currentPage >= errorEntries.length - 1) return
					currentPage += 1
					const pageEmbed = buildErrorEmbed(errorEntries[currentPage], currentPage, errorEntries.length)
					const navRow = buildErrorNavRow(currentPage, errorEntries.length)
					replyMessage.edit({ embeds: [pageEmbed], components: [navRow as any] })
				...
				case "info_errors_back": {
					...
					replyMessage.edit({ embeds: [infoEmbed], components: [navRow as any] })
					break
				}
			}
		})
		...
		collector.on("end", async () => {
			...
			try {
				replyMessage.edit({ components: [row as any] })
			} catch {
